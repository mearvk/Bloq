...
public final class Bloqapmlpopulator
{
	...
	public ArrayList<Apmlmodelfile> getapmlmodelfiles(File apmlfile, String apmltag) throws Exception
	{
		...
		for (int i = 0; i < xparam.getnodecount(); i++)
		{
			switch (apmltag) //would probably need a 'parent' reference is all these cases given how it's been implemented
			{
				...
				case "//callback":
					return docallbacktags(xparam);

				case "//definitions":
					return dodefinitiontags(xparam);

				case "//dynamiclistener":
					return dodynamiclistenertags(xparam);

				case "//listener":
					return dolistenertags(xparam);

				case "//object":
					return doobjecttags(xparam);

				case "//subscriber":
					return dosubscribertags(xparam);

				case "//system":
					return dosystemtags(xparam);
			}
		}
	...
	private ArrayList<Apmlmodelfile> dodefinitiontags(Bloqxpathparameter xparam) throws Exception
	{
		...
		for (int index = 0; index < xparam.getnodecount(); index++)
		{
			...
			try
			{
				modelfile.apmllisteners = this.getlisteners(xparam, index);
			}
			catch (Exception e)
			{/*LOGGER.log(Level.WARNING, e.getMessage(), e);*/}
			...
			try
			{
				modelfile.apmlobjects = this.getobjects(xparam, index);
			}
			catch (Exception e)
			{/*LOGGER.log(Level.WARNING, e.getMessage(), e);*/}
	...
	private ArrayList<Apmlmodelfile> dolistenertags(Bloqxpathparameter xparam)
	{
		...
		for (int index = 0; index < xparam.getnodecount(); index++)
		{
			...
			try
			{
				modelfile.apmlsubscribers = this.getsubscribers(xparam, index);
			}
			catch (Exception e)
			{/*LOGGER.log(Level.WARNING, e.getMessage(), e);*/}
