...
public abstract class Standardabstractapmlcompiler
{
	...
	public Standardabstractapmlcompiler()
	{
		try
		{
			...
			if (!this.fileguardian.buildoutdirfile.exists())
				this.fileguardian.buildoutdirfile.mkdirs();

			if (!this.fileguardian.libsdirfile.exists())
				this.fileguardian.libsdirfile.mkdirs();

			if (!this.fileguardian.loggingdirfile.exists())
				this.fileguardian.loggingdirfile.mkdirs();

			if (!this.fileguardian.manifestdirfile.exists())
				this.fileguardian.manifestdirfile.mkdirs();

			if (!this.fileguardian.sourceoutdirfile.exists())
				this.fileguardian.sourceoutdirfile.mkdirs();

			if (!this.fileguardian.tempdirfile.exists())
